...
<body>
    ...
    $countSql = "SELECT COUNT(*) as total FROM video WHERE id LIKE ? OR judul LIKE ?";
    $countStmt = $koneksi->prepare($countSql);
    $countStmt->bind_param("is", $search, $search);
    $countStmt->execute();
    ...
    $sql = "SELECT * FROM video WHERE id LIKE ? OR judul LIKE ? LIMIT ?, ?";
    ...
    <div class="main-content">
        <h1><i class="fa fa-circle-play"> </i> Video Youtube <i class="fa fa-sun spin"> </i> </h1>
        <h3> Admin / Video Kegiatan</h3>
        ...
        <div class="cari">
            ...
            <a class="btn btn-primary" href="create_video.php" role="button"> <i class="fa fa-plus"></i> Tambah Data</a>
            <a class="btn btn-secondary" href="video.php" role="button"><i class="fa fa-rotate-right"></i> Refresh</a>
        </div>
        <table class="table table-hover">
            ...
            <tbody>
                ...
                ?>
                        <tr>
                            <td><?php echo $id; ?></td>
                            <td><?php echo htmlspecialchars($baris['judul']); ?></td>
                            <td><?php echo htmlspecialchars($baris['link_youtube']); ?></td>

                            <td>
                                <a class="btn btn-success" href="edit_video.php?id=<?php echo $baris['id']; ?>" role="button"><i class="fa fa-pen-to-square"></i></a>
                                <a class="btn btn-warning" href="lihat_video.php?id=<?php echo $baris['id']; ?>" role="button"><i class="fa fa-eye"></i></a>
                                <a class="btn btn-dark" href="hapus_video.php?id=<?php echo $baris['id']; ?>" role="button"><i class="fa fa-trash-can"></i></a>
                            </td>
                        </tr>
                <?php
                    }
